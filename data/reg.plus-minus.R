#----------------------------------------------------------------------------------------------
# File: reg.plus-minus.R
# Date: 07-03-2012
# Author: Eric Nantz
# URL: https://github.com/thercast/nhl_analysis/blob/master/data/reg.plus-minus.R
# Email: theRcast@gmail.com
# Purpose: Import NHL regular season plus-minus data files from Hockey Summary Project
...
#reg.data.dir <- "~/Dropbox/hockey_data_files/regular_season_files"


reg.plusminus.files <- list.files(path=reg.data.dir, pattern="^.*-plusminus\\.dat", full.names=TRUE, recursive=TRUE)

reg.plusminus.filedata <- file.info(reg.plusminus.files)
filedata.sub <- reg.plusminus.filedata[, 1:2]
...
reg.plusminus.files <- setdiff(reg.plusminus.files, files.to.remove)
...
# blah <- ldply(reg.plusminus.files,
#               file.messy,
#               messychar=";")
# 
# blah <- ldply(reg.plusminus.files,
...
reg.plusminus <- tmp[,1:length(col.classes)]

names(reg.plusminus) <- col.names

# get snapshot of unique values for each variable
#lapply(reg.plusminus, function(x) sort(unique(x))[1:30])
