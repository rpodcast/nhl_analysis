...
for (team in teams[1]) { #team
    ...
    print(out.string)
    cat(out.string, "\n", file="~/hockey_workspaces/register.log.txt", append=TRUE)
...
#       team.player.table.names <- names(table.player.stats)[player.table.ind]
#       team.goalie.table.names <- names(table.goalie.stats)[goalie.table.ind]

      team.player.table <- as.data.frame(table.player.stats["skaters"])
      team.player.table$team <- team
      
      team.goalie.table <- as.data.frame(table.goalie.stats["goalies"])
     ...
     year.goalie.register.data <- paste("goalie.register.data.", team, sep="")
